	...
	class Stocks {
		...
		function __construct() {
		    ...
		    if( isset($_POST['action']) ) {
		        ...
		        $this->stock_alert 	= isset($params['stock_alert']) ? $params['stock_alert'] : 'all';
		        $this->fast_moving 	= isset($params['fast_moving']) ? $params['fast_moving'] : 'all';
		        ...
		        $this->stock_alert 	= isset( $_GET['stock_alert'] ) ? $_GET['stock_alert']  : 'all';
		        $this->fast_moving 	= isset( $_GET['fast_moving'] ) ? $_GET['fast_moving']  : 'all';
		...
		function stock_list_pagination_total( $args ) {
	    	...
	    	$page_arg['fast_moving'] = $this->fast_moving;
		    ...
		    if($this->stock_alert != 'all'){

		    	$condition .= " AND is_alert = '1'";
		    }


		    if($this->fast_moving != 'all'){
		    ...
		    $query = "SELECT * from (
		    	SELECT *,(case when (balance_stock-stock_alert) > 0  then 0 else 1 end  ) as is_alert,balance_stock-stock_alert as alert from ( SELECT lot_table.*,(case when final_sale_tab.tot_sale is null then 0 else final_sale_tab.tot_sale end ) as tot_sale, (case when final_sale_tab.tot_sale is null then lot_table.stock_in else lot_table.stock_in - final_sale_tab.tot_sale end ) as balance_stock from (select lot_table.*,(case when stock_table.stock_in is null then 0 else stock_table.stock_in end )as stock_in from (select id,cgst,sgst,selling_price,`brand_name`,`product_name`,`hsn`,`stock_alert` from wp_shc_lots WHERE active = 1) as lot_table left join (select (case when sum(stock_count) is null then 0 else sum(stock_count) end )as stock_in,lot_number from wp_shc_stock WHERE active=1 GROUP by lot_number ) as stock_table on lot_table.id = stock_table.lot_number ) as lot_table left join (select (sum(total_sale_unit)) as tot_sale,lot_id from ( select (case when return_table.return_unit is null then sale_table.sale_unit else (sale_table.sale_unit - return_table.return_unit) end) as total_sale_unit,sale_table.lot_id from (SELECT (case when sum(sale.sale_unit ) is null then 0 ELSE sum(sale.sale_unit ) end) as sale_unit ,(case when (sale.lot_id) is null then 0 else (sale.lot_id) end) as lot_id FROM wp_shc_sale_detail as sale WHERE sale.active =1 group by lot_id) as sale_table left join (SELECT (case when sum(return_tab.return_unit ) is null then 0 ELSE sum(return_tab.return_unit ) end) as return_unit ,(case when (return_tab.lot_id) is null then 0 else (return_tab.lot_id) end) as lot_id FROM wp_shc_return_items_details as return_tab WHERE return_tab.active =1 group by return_tab.lot_id)  as return_table on sale_table.lot_id = return_table.lot_id UNION all select (case when ws_return_table.return_unit is null then ws_sale_table.sale_unit else (ws_sale_table.sale_unit - ws_return_table.return_unit) end ) as total_sale_unit,ws_sale_table.lot_id from (SELECT (case when sum(ws_sale.sale_unit ) is null then 0 ELSE sum(ws_sale.sale_unit ) end) as sale_unit ,(case when (ws_sale.lot_id) is null then 0 else (ws_sale.lot_id) end) as lot_id FROM wp_shc_ws_sale_detail as ws_sale WHERE ws_sale.active =1 group by lot_id) as ws_sale_table left join (SELECT (case when sum(ws_return_tab.return_unit ) is null then 0 ELSE sum(ws_return_tab.return_unit ) end) as return_unit ,(case when (ws_return_tab.lot_id) is null then 0 else (ws_return_tab.lot_id) end) as lot_id FROM wp_shc_ws_return_items_details as ws_return_tab WHERE ws_return_tab.active =1 group by ws_return_tab.lot_id) as ws_return_table on ws_sale_table.lot_id = ws_return_table.lot_id) as invoice_table GROUP by lot_id) final_sale_tab on final_sale_tab.lot_id = lot_table.id ) as final_tab) AS tab WHERE tab.id > 0 ${condition}";
		    ...
		    $data['result']    = $wpdb->get_results( $query . " ORDER BY ${args['orderby_field']} ${args['order_by']} ${args['second_orderby_field']} ${args['second_order_by']} LIMIT ${offset}, ${args['items_per_page']}" );
//var_dump($query . " ORDER BY ${args['orderby_field']} ${args['order_by']} ${args['second_orderby_field']} ${args['second_order_by']} LIMIT ${offset}, ${args['items_per_page']}");	
		...
		// function stock_list_pagination_dashboard( $args ) {
		//     global $wpdb;
		//     $stock_table =  $wpdb->prefix.'shc_stock';
		//     $lots_table =  $wpdb->prefix.'shc_lots';
		//     $stock_details = $wpdb->prefix.'shc_ws_sale_detail';
	 ...
	 //    	$page_arg['count'] = $this->count;
		...
		//     $query = "SELECT *,(case when (balance_stock-stock_alert) > 0  then 0 else 1 end  ) as is_alert,balance_stock-stock_alert as alert from ( SELECT lot_table.*,(case when final_sale_tab.tot_sale is null then 0 else final_sale_tab.tot_sale end ) as tot_sale, (case when final_sale_tab.tot_sale is null then lot_table.stock_in else lot_table.stock_in - final_sale_tab.tot_sale end ) as balance_stock from (select lot_table.*,(case when stock_table.stock_in is null then 0 else stock_table.stock_in end )as stock_in from (select id,cgst,sgst,selling_price,`brand_name`,`product_name`,`hsn`,`stock_alert` from wp_shc_lots WHERE active = 1) as lot_table left join (select (case when sum(stock_count) is null then 0 else sum(stock_count) end )as stock_in,lot_number from wp_shc_stock WHERE active=1 GROUP by lot_number ) as stock_table on lot_table.id = stock_table.lot_number ) as lot_table left join (select (sum(total_sale_unit)) as tot_sale,lot_id from ( select (case when return_table.return_unit is null then sale_table.sale_unit else (sale_table.sale_unit - return_table.return_unit) end) as total_sale_unit,sale_table.lot_id from (SELECT (case when sum(sale.sale_unit ) is null then 0 ELSE sum(sale.sale_unit ) end) as sale_unit ,(case when (sale.lot_id) is null then 0 else (sale.lot_id) end) as lot_id FROM wp_shc_sale_detail as sale WHERE sale.active =1 group by lot_id) as sale_table left join (SELECT (case when sum(return_tab.return_unit ) is null then 0 ELSE sum(return_tab.return_unit ) end) as return_unit ,(case when (return_tab.lot_id) is null then 0 else (return_tab.lot_id) end) as lot_id FROM wp_shc_return_items_details as return_tab WHERE return_tab.active =1) as return_table on sale_table.lot_id = return_table.lot_id UNION all select (case when ws_return_table.return_unit is null then ws_sale_table.sale_unit else (ws_sale_table.sale_unit - ws_return_table.return_unit) end ) as total_sale_unit,ws_sale_table.lot_id from (SELECT (case when sum(ws_sale.sale_unit ) is null then 0 ELSE sum(ws_sale.sale_unit ) end) as sale_unit ,(case when (ws_sale.lot_id) is null then 0 else (ws_sale.lot_id) end) as lot_id FROM wp_shc_ws_sale_detail as ws_sale WHERE ws_sale.active =1 group by lot_id) as ws_sale_table left join (SELECT (case when sum(ws_return_tab.return_unit ) is null then 0 ELSE sum(ws_return_tab.return_unit ) end) as return_unit ,(case when (ws_return_tab.lot_id) is null then 0 else (ws_return_tab.lot_id) end) as lot_id FROM wp_shc_ws_return_items_details as ws_return_tab WHERE ws_return_tab.active =1) as ws_return_table on ws_sale_table.lot_id = ws_return_table.lot_id) as invoice_table GROUP by lot_id) final_sale_tab on final_sale_tab.lot_id = lot_table.id ) as final_tab WHERE final_tab.id > 0";
		...
		//     $data['result']    = $wpdb->get_results( $query . " ORDER BY is_alert DESC  LIMIT ${offset}, ${args['items_per_page']}" );
		...
		function stock_list_pagination( $args ) {
		    ...
		    $query 				= "SELECT l.*, s.id as stock_id, s.stock_count, s.selling_total,s.modified_at as modified, s.created_at as stock_created FROM ${stock_table} as s JOIN ${lots_table} as l ON s.lot_number = l.id WHERE s.active = 1 and l.active=1 ${condition}";
		    ...
		    $data['result']     = $wpdb->get_results( $query . "ORDER BY  ${args['orderby_field']} DESC LIMIT ${offset}, ${args['items_per_page']}" );
